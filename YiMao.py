...
# Convert datatype to values to do regression
...
# delete records with no target values (values with *)
df_new=df_new[df_new['Assessment Score']!='*']
# Select features and select data sets
X = df_new.iloc[:, np.r_[1:3,5:8,10:12,16:23]]
y = df_new.iloc[:, 14]
X_1, X_test, y_1, y_test = train_test_split(X, y)
X_train, X_val, y_train, y_val = train_test_split(X_1, y_1)
